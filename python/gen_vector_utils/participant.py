...
def generate_participant_step1_group(tc_id_start):
    valid_cases = []
    error_cases = []
    tc_id = 0
...
def generate_participant_step1_vectors():
    groups = []
    tc_id = 0
    group = generate_participant_step1_group(tc_id)
...
def generate_participant_step2_group(tc_id_start):
    valid_cases = []
    error_cases = []
    tc_id = 0
...
def generate_participant_step2_vectors():
    groups = []
    tc_id = 0
    group = generate_participant_step2_group(tc_id)
    tc_id += len(group["valid_test_cases"]) + len(group["error_test_cases"])
    groups.append(group)
    return {
...
def generate_participant_finalize_group(tc_id_start):
    hostseckeys = hex_list_to_bytes(HOSTSECKEYS_HEX[:3])
    hostpubkeys = [chilldkg.hostpubkey_gen(sk) for sk in hostseckeys]
    params = chilldkg.SessionParams(hostpubkeys, 2)
    randoms = hex_list_to_bytes(RANDOMS_HEX[:3])
...
def generate_participant_finalize_vectors():
    groups = []
    tc_id = 0
    group = generate_participant_finalize_group(tc_id)
    tc_id += len(group["valid_test_cases"]) + len(group["error_test_cases"])
    groups.append(group)
    return {
...
def generate_participant_investigate_group(tc_id_start):
    hostseckeys = hex_list_to_bytes(HOSTSECKEYS_HEX[:3])
    hostpubkeys = [chilldkg.hostpubkey_gen(sk) for sk in hostseckeys]
    params = chilldkg.SessionParams(hostpubkeys, 2)
    randoms = hex_list_to_bytes(RANDOMS_HEX[:3])
...
def generate_participant_investigate_vectors():
    groups = []
    tc_id = 0
    group = generate_participant_investigate_group(tc_id)
